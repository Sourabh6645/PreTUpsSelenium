...
import angular.feature.NetworkServices_Feature;

@ModuleManager(name = Module.PREREQUISITE_NetworkServices)
public class NetworkServices_TestScript extends BaseTest {
	@Test(dataProvider = "categoryData")
	// @TestManager(TestKey = "PRETUPS-000") // TO BE UNCOMMENTED WITH JIRA TEST

	// CASE 01
	public void A_01_Test_NetworkServices_Modify(String UserCat, String LoginId, String Password) {
		final String methodName = "A_01_Test_NetworkServices_Modify";
		Log.startTestCase(methodName, UserCat, LoginId, Password);
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("PreTUPSNetworkServices01");
		...
		NetworkServices_Feature NS = new NetworkServices_Feature(driver);

		NS.ModifyNetworkServices(LoginId, Password);
	...
	// CASE 02
	public void A_02_Test_NetworkServices_ResetmodifyServices_And_ResetFilter_andValidateErrormsg(String UserCat, String LoginId,
			String Password) {
		final String methodName = "A_02_Test_NetworkServices_ResetmodifyServices_And_ResetFilter_andValidateErrormsg";
		Log.startTestCase(methodName, UserCat, LoginId, Password);
		CaseMaster CaseMaster = _masterVO.getCaseMasterByID("PreTUPSNetworkServices02");
		currentNode = test.createNode(MessageFormat.format(CaseMaster.getExtentCase(), UserCat))
				.assignCategory(TestCategory.PREREQUISITE);

		NetworkServices_Feature NS = new NetworkServices_Feature(driver);

		NS.ResetmodifyServices_And_ResetFilter(LoginId, Password);
