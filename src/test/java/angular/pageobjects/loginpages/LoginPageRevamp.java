...
public class LoginPageRevamp {
	...
	/* ---Change By Shubham Yadav---- */
	@FindBy(id = "userPassword")
	private WebElement vmsPassword;
	...
	public void enterVMSUIPassword(String Password) {
		Log.info("Trying to enter Password");
		try {
			wait.until(ExpectedConditions.visibilityOf(vmsPassword));
			vmsPassword.clear();
			vmsPassword.sendKeys(Password);
		} catch (Exception e) {
			wait.until(ExpectedConditions.visibilityOf(vmsPassword));
			vmsPassword.clear();
			vmsPassword.sendKeys(Password);
