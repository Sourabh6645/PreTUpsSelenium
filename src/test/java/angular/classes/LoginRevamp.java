...
public class LoginRevamp {
	...
	/* ----Function Change Added By Shubham LogIn Using SuperAdmin---- */
	
	public String UserLoginSuperAdmin(WebDriver driver,String loginId,String password) {
		/* Loading Properties File */
		String MasterSheetPath = _masterVO.getProperty("DataProvider");
		...
		ExcelUtility.setExcelFile(MasterSheetPath, ExcelI.MASTER_SHEET_NAME);
		String LANGUAGE=_masterVO.getMasterValue(MasterI.LANGUAGE);
		String URL=_masterVO.getMasterValue(MasterI.WEBVMS_URL);
		driver.get(URL);
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		LoginPageRevamp loginPage = new LoginPageRevamp(driver);
		try {
		...
		loginPage.enterLoginID(loginId);
		loginPage.enterVMSUIPassword(password);
		loginPage.clickLoginButton();
			String ErrorMessage = loginPage.getErrorMessage();
			//loginPage.clickReloginButton();
			return ErrorMessage;
